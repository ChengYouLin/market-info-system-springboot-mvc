...
public interface HavePointsRespository extends JpaRepository<HavePoints, Integer> {
    ...
    @Query("SELECT new com.example.dbms_group2.model.DTO.PointDTO(h.points) " +
            "FROM HavePoints h " +
            "WHERE h.user.gmail = :gmail AND h.market.marketId = :marketId")
    List<PointDTO> findTotalPoint(@Param("gmail") String email, @Param("marketId") int marketId);

    @Modifying
    @Query("UPDATE HavePoints h SET h.points = h.points + 1 " +
            "WHERE h.user.gmail = :gmail AND h.market.marketId = :marketId")
